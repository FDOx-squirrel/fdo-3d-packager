...
| --strict + CIIC 81s absichtlicher id-Override zusammen | Erwartet, kein Bug: der reale --strict --publish-only --from fetch ...-Vollauf (2026-09-09) hat am Ende exit 1 geliefert (--strict: warnings present, failing.), obwohl beide Slugs komplett durchgelaufen UND sauber aufgeraeumt sind (fdo-squirrel round-trip confirmed fuer beide, --publish-only hat beide fertigen Bundles korrekt uebriggelassen) -- die ausloesende Warning: ist exakt die oben schon bestaetigte, gewollte id-Override-Warnung (reservierte Zenodo-DOI). --strict wertet nur den Gesamt-Exit-Code am Ende aus (had_warning-Sammlung in run_selection_once()/_run_fetch_then_rest()), bricht aber keinen Schritt oder Slug deswegen vorzeitig ab. Fuer einen echten kuratierten Lauf mit einem bekannten, gewollten Override ist --strict damit eher ein CI-/Regressions-Werkzeug als ein Produktions-Gate -- fuer den naechsten CIIC-81-Lauf kann Flo es weglassen, ohne etwas zu verlieren | 2026-09-09, Befund aus echtem Vollauf |
